...
# LSTM_model
class Lstm_enso(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        super(Lstm_enso, self).__init__()
...
def train(model, criterion, optimizer, train_data):
    ...
    for i, batch in enumerate(train_data):
        ...
        tao1_t = torch.autograd.grad(TAO1_pred, src_t, grad_outputs=torch.ones_like(TAO1_pred), retain_graph=True, create_graph=True)[0]

        f_2 = T_t - 150*TAO1_pred + 250*TAO1_pred*(src_t - 5/12) + 1.2 * (SSTA_pred ** 3)
        g_2 = tao1_t - 0.036*SSTA_pred + 2*TAO1_pred

        # f_2 = 150 * TAO1_pred - 250 * TAO1_pred * (src_t - 5 / 12) - 1.2 * (SSTA_pred ** 3)
        # g_2 = 0.036 * SSTA_pred - 2 * TAO1_pred

        loss1 = criterion(SSTA_pred, tgt_SSTA) + criterion(HA_pred, tgt_HA) + criterion(TAO1_pred, tgt_TAO1) + criterion(TAO2_pred, tgt_TAO2)
        # loss1 = criterion(SSTA_pred, tgt_SSTA) + criterion(TAO1_pred, tgt_TAO1)
        loss2 = torch.mean(torch.pow((f_2), 2)) + torch.mean(torch.pow((g_2), 2))
        # loss3 = criterion(T_t, f_2) + criterion(tao1_t, g_2)
...
def main():
    ...
    enso_pred = Lstm_enso(input_size, hidden_size, num_layers, output_size)
    enso_pred = enso_pred.to(device)

    optimizer = torch.optim.Adam(enso_pred.parameters(), lr=0.003)
    criterion = nn.MSELoss()
    # criterion = nn.L1Loss()

    for i in range(epochs):
        epoch_loss = train(enso_pred, criterion, optimizer, train_loader)
        epoch_loss_all.append(epoch_loss)
        print("epoch: {} PILSTM2_1_20_train loss: {}".format(i, epoch_loss))
        if epoch_loss < best_loss:
            # best_loss = epoch_loss
            model_name = r'/data/home/scv6730/run/model/LSTM/2/PILSTM2_1_20_model_{0:.5f}.pt'.format(epoch_loss)
